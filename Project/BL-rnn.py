...
from keras.layers import LSTM, Dense, Activation, ThresholdedReLU, MaxPooling2D, Embedding, Dropout, CuDNNLSTM, CuDNNGRU, Bidirectional, SimpleRNN
...
model_rnn.add(SimpleRNN(16, input_shape=(X.shape[1], X.shape[2]), return_sequences=True))
...
## train
hist = model_rnn.fit(X_train, y_train, validation_data=(X_valid, y_valid), 
...
plt.legend()
plt.show()
plt.savefig("train-val-MSE-curve-SimpleRNN.png")


pred = model_rnn.predict(X_test)

submission = pd.DataFrame(pred,columns=['item_cnt_month'])
submission.to_csv('rnn_submission.csv',index_label='ID')
